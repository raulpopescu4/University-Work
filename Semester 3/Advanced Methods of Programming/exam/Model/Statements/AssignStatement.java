...
public class AssignStatement implements iStatement {
    ...
    public ProgramState execute(ProgramState state) throws MyException {
        ...
        if (currentSymbolTable.isDefined(this.id)) {
            Value value = this.expression.evaluate(currentSymbolTable, heapTable);
            Type typeID = (currentSymbolTable.lookUp(this.id)).getType();

            if (value.getType().equals(typeID))
                currentSymbolTable.update(this.id, value);
