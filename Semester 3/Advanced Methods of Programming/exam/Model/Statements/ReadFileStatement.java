...
public class ReadFileStatement implements iStatement{
    ...
    public ProgramState execute(ProgramState state) throws MyException {
        ...
        Value value = currentSymbolTable.lookUp(this.variableName);
        if (!value.getType().equals(new IntType()))
            throw new MyException("The expression must be a IntType!");

        MyiHeap heapTable = state.getHeapTable();
        value = this.expression.evaluate(currentSymbolTable, heapTable);
        ...
        try {
            ...
            currentSymbolTable.add(this.variableName, new IntValue(Integer.parseInt(currentLine)));
