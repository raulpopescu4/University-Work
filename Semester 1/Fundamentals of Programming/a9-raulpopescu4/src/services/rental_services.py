...
class RentalServices:
    ...
    def generate_random_rental_data(self):
        ...
        client_data_list = list(self._client_repository.get_all())
        movie_data_list = list(self._movie_repository.get_all())
    ...
    def remove_rentals_client_id(self, client_id):
        ...
        :param client_id:
        """
        rentals = self._rental_repository.get_all()
        rental_ids_removed = []
        ...
        for rental_id in rental_ids_removed:
            self._rental_repository.remove(rental_id)
    ...
    def rent_new_movie(self, rental_id, movie_id, client_id, rental_date):
        ...
        try:
            movie = self._movie_repository.search(movie_id)
        except RepositoryException as exception:
            raise RepositoryException(str(exception))

        try:
            client = self._client_repository.search(client_id)
        except RepositoryException as exception:
            raise RepositoryException(str(exception))

        client_can_rent = True
        rentals_list = self._rental_repository.get_all()
        ...
        undo_call = Call(self._rental_repository.remove, rental.get_id())
        redo_call = Call(self._rental_repository.add, rental)
    ...
    def return_a_movie(self, rental_id, return_date):
        ...
        :return:
        """
        rental = self._rental_repository.search(rental_id)
        ...
        self._rental_repository.update(modified_rental)

        undo_call = Call(self._rental_repository.update, rental_before_return)
        redo_call = Call(self._rental_repository.update, modified_rental)
    ...
    def get_most_rented_movies(self):
        ...
        for movie_id in movie_id_and_days_counter:
            movie_title = self._movie_repository.search(movie_id).get_title()
    ...
    def get_most_active_clients(self):
        ...
        for client_id in client_id_and_days_counter:
            client_name = self._client_repository.search(client_id).get_name()
